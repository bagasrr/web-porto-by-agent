...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-[var(--bg)]">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex flex-col w-64 bg-[var(--bg-secondary)] border-r border-[var(--border-color)] transition-transform duration-300 md:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {/* Logo */}
        <div className="p-6 border-b border-[var(--border-color)]">
          <Link href="/" className="text-xl font-bold font-[family-name:var(--font-display)] text-[var(--text)] hover:text-[var(--accent)] transition-colors">
        ...
        <div className="p-4 border-t border-[var(--border-color)] space-y-2">
          <Link href="/" target="_blank" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text)] hover:bg-[var(--surface)] transition-all">
            <HiHome size={17} className="text-[var(--text-muted)]" />
            View Portfolio
          </Link>
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-[var(--danger)] hover:bg-[var(--surface)] transition-all">
            <HiLogout size={17} />
            Logout
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col md:ml-64">
        ...
        <header className="sticky top-0 z-20 flex items-center justify-between h-16 px-4 md:px-6 bg-[var(--bg)]/90 backdrop-blur-xl border-b border-[var(--border-color)]">
          ...
          <h1 className="text-sm font-semibold text-[var(--text-secondary)] hidden md:block">Admin Dashboard</h1>
